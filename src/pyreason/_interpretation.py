"""The reference reasoning core — the fixed-point temporal loop over one
explicit Interpretation state.

Behavior target: the pinned default-settings engine (oracle
scripts/interpretation/interpretation.py, the non-parallel non-fp variant, at
e1a94af3), reproduced meaning-for-meaning as pure Python over plain
containers. The Interpretation object IS the state: worlds, predicate maps,
pending fact/rule queues, and the trace live on it, and the loop functions
below read and mutate nothing else. Every list is order-preserving because
queue order is trace-row order and trace-row order is frame-row order.

The pinned timestep shape (interpretation.py:242-686):
...
class Interpretation:
    ...
    def __init__(self, graph, ipl, annotation_functions, head_functions,
                 ...
                 specific_edge_labels, closed_world_predicates):
        self.graph = graph
        self.ipl = ipl
...
def _apply_fact(interp, kind, i, t, fp_cnt, convergence_state):
    ...
    pinned per-kind fact loops (interpretation.py:285-343/362-418)."""
    if kind == 'node':
        queue, trace = interp.facts_to_be_applied_node, interp.facts_to_be_applied_node_trace
        interpretations, predicate_map = interp.interpretations_node, interp.predicate_map_node
        ...
        interpretations, predicate_map = interp.interpretations_edge, interp.predicate_map_edge
        ...
        while in_loop:
            ...
            for idx, entry in enumerate(interp.rules_to_be_applied_edge):
                if entry[0] == t:
                    ...
                    edges_added, changes = _add_edges(interp, sources, targets, edge_l, t)
                    convergence_state['changes_cnt'] += changes
...
def _ground_rule(interp, rule, extended_ann_fn, t):
    """Ground one rule against the current interpretation
    (interpretation.py:809-1281). Returns (applicable node rules, applicable
    edge rules): per satisfied head grounding, the annotation inputs and the
    per-clause qualified nodes/edges the trace rows carry."""
...
def _update_component(interp, interpretations, predicate_map, comp, na, rule_trace,
    ...
    # A label new to this world starts at the vacuous bound
...
def _add_node(interp, node):
    interp.nodes.append(node)
    interp.neighbors[node] = []
    interp.reverse_neighbors[node] = []
    interp.interpretations_node[node] = World([])


def _add_edge(interp, source, target, l, t):
    if source not in interp.nodes:
        _add_node(interp, source)
    if target not in interp.nodes:
        _add_node(interp, target)
    ...
    if edge not in interp.edges:
        ...
        if l.get_value() != '':
            interp.interpretations_edge[edge] = World([l])
            interp.num_ga[t] += 1
            ...
            interp.interpretations_edge[edge] = World([])
    else:
        if l not in interp.interpretations_edge[edge].world and l.get_value() != '':
            new_edge = True
            interp.interpretations_edge[edge].world[l] = interval.closed(0, 1)
            interp.num_ga[t] += 1
            if l in interp.predicate_map_edge:
                interp.predicate_map_edge[l].append(edge)
            else:
...
def _add_edges(interp, sources, targets, l, t):
    changes = 0
    edges_added = []
